...
:fun_start
    cd %DIR%
    start werl -pa ebin  +P 1024000  -setcookie %COOKIE%  -hidden -kernel inet_dist_listen_min %inet_dist_listen_min% inet_dist_listen_max %inet_dist_listen_max%  -name guess_new@192.168.0.79  -s main start -extra local
    goto where_to_go 
...
:fun_sync
	cd %DIR%
	Set SVN=D:\svn\bin
	copy .\include\error_msg.hrl .\..\..\..\program\guess_new\main\error_msg
	copy .\protobuffs\proto\*.proto .\..\..\..\program\guess_new\main\proto
	cd .\..\..\..\program\guess\main 
	"%SVN%\TortoiseProc.exe"/command:update /path:"D:\program\guess_new\main\proto\*"/closeoned:0
	"%SVN%\TortoiseProc.exe"/command:commit /path:"D:\program\guess_new\main\*"/closeoned:0
	goto where_to_go
:end
