...
handle_info(hit, Role = #role{status = ?status_great_match, great_match = #great_match{num = Num}}) ->
    NewRole = case Num > 0 of
        true ->
            do_hit(Role);
        _ ->
...
%% 0点更新
do_zero_flush(Role = #role{great_match = #great_match{week_score = Score}}) -> 
    Now = date:unixtime(),
    ...
    Role2#role{daily_value = [], login_time = Now, daily_kill = 0, great_match = NewGreat}.
...
enter_room(Role, great) ->
    Role;
enter_room(Role = #role{great_match = Great}, great) ->
    case great_match_mgr:enter_room(Role#role{great_match = Great#great_match{num = 2000}}) of
        {ok, #m_2001_toc{animals = List}, NewRole} ->
            put(animal_list, List),
            NewRole;
        {false, _Reason} ->
            Role
    ...
    case packet:unpack(Cmd, DateStatus, NewBin, toc) of
        {ok, Cmd, Data} ->
            case Cmd of
                ...
                2009 ->
                    #m_2009_toc{role_id = RoleID, ids = List} = Data,
                    {HitId, _} = get(hit_id),
                    OldList = get(animal_list),
                    List1 = [Id||#p_animal_die{id = Id} <-List],
                    NewList = [Animal||Animal = #p_animal{id = Id} <-OldList, not lists:member(Id, List1)],
                ...
                1321 ->
                    ...
                    ok;
                2021 ->
                    #m_2021_toc{role_id = Id, type = Type, to_id = ToId} = Data,
                    do_back_expression(Role, Id, Type, ToId),
                    ok;
%%                1147 ->
