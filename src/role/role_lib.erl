...
%% Vip登陆奖励抽奖
do_vip_login_reward(Role = #role{vip = Vip}) ->
    #vip_welfare{login = Num} = vip:get_vip_welfare(Vip),
    Value = get_value(Role, ?daily_vip_login),
    case Value < Num  of
        true -> 
            {Type, Num1, _} = login_vip_reward(),
            {ok, NewRole} = do_add(Role, [{Type, Num1}]),
            NewRole1 = add_value(NewRole, ?daily_vip_login),
...
%% 随机奖励
login_vip_reward() ->
    List = [{trumpet, 1, 1000}, {coin, 30000, 1000}, {gold, 20, 1000}, {gold, 40, 500},  {ice, 2, 1000}, {locking, 2, 1000}, {tel_fare, 20, 100}, {auto, 1, 1000}],
    sys_rand:rand_list(List, 3).
...
%% 获取背包信息
get_bag_info(#role{item = #role_item{ice = Ice, horn = Horn, rage = Rage, trumpet = Trumpet, locking = Locking, auto = Auto, lollipop = Lollipop, tel_fare = Tel, red_bag = RedBag}, candy = Candy, lolly = Lolly, self_horn = SelfHorn, xin_card = Xin, nian_card = Nian, kuai_card = Kuai, le_card = Le, gold_pick = Pick, active_card = Card, box1 = Box1, box2 = Box2, box3 = Box3}) ->
    [#p_assets{type = ice, num = Ice}, #p_assets{type = horn, num = Horn}, #p_assets{type = rage, num = Rage}, #p_assets{type = trumpet, num = Trumpet}
        ...
        ,#p_assets{type = gold_pick, num = Pick}, #p_assets{type = active_card, num = Card}, #p_assets{type = box1, num = Box1}, #p_assets{type = box2, num = Box2}
        ,#p_assets{type = box3, num = Box3}
    ].
...
%% box1
do_cost(Role = #role{box1 = OldValue}, [{box1, Value} | L], List) when OldValue >= Value-> 
    do_cost(Role#role{box1 = OldValue - Value}, L, [{box1, OldValue - Value} | List]);

do_cost(Role = #role{box2 = OldValue}, [{box2, Value} | L], List) when OldValue >= Value-> 
    do_cost(Role#role{box2 = OldValue - Value}, L, [{box2, OldValue - Value} | List]);

do_cost(Role = #role{box3 = OldValue}, [{box3, Value} | L], List) when OldValue >= Value-> 
    do_cost(Role#role{box3 = OldValue - Value}, L, [{box3, OldValue - Value} | List]);
...
do_add(Role = #role{item = Item = #role_item{red_bag = OldValue}}, [{red_bag, Value} | L], List) -> 
    ...
    do_add(NewRole, L, [{red_bag, OldValue + Value} | List]);
...
%% 宝箱
do_add(Role = #role{box1 = OldValue}, [{box1, Value} | L], List) -> 
    do_add(Role#role{box1 = OldValue + Value}, L, [{box1, OldValue + Value} | List]);
do_add(Role = #role{box2 = OldValue}, [{box2, Value} | L], List) -> 
    do_add(Role#role{box2 = OldValue + Value}, L, [{box2, OldValue + Value} | List]);
do_add(Role = #role{box3 = OldValue}, [{box3, Value} | L], List) -> 
    do_add(Role#role{box3 = OldValue + Value}, L, [{box3, OldValue + Value} | List]);
    ...
    case RedBag >= Need of
        true ->
            ...
            case N >= Max of
                ...
                _ ->
                    Value = min(500000, (RedBag div Need) * Need * 10),  %% 分
                        ...
                        true ->
                            ...
                            case db:exec("insert into red_packet_log(role_id, cny, order_id, time) value(?, ?, ?, ?)", [RoleID, Value, ParTraNo, Now]) of
                                ok -> 
                                    ...
                                    {ok, NewRole} = do_cost(Role, [{red_bag, trunc(Value/10)}]),  %% 角
...
%% 兑换红包
reward_red_bag(_Role = #role{red_openid = ""}, _) ->
    {false, ?error_red_bag_code};
reward_red_bag(Role = #role{vip = Vip, red_openid = OpenId, role_id = RoleID, exchange = Exchange, channel = Channel}, Value) ->
    N = get_value(Role, ?daily_red_bag),
    #vip_welfare{max_red = Max} = vip:get_vip_welfare(Vip),
    case N >= Max of
        true ->
            ...
            case db:exec("insert into red_packet_log(role_id, cny, order_id, time) value(?, ?, ?, ?)", [RoleID, Value, ParTraNo, Now]) of
                ok -> 
                    case send_red_bag(ParTraNo, OpenId, Value) of
                        {ok, ParTraNo} ->
                            db:exec("update red_packet_log set status = ? where order_id = ?", [1, ParTraNo]),
                            NewRole1 = add_value(Role, ?daily_red_bag),
                            case Channel of
                                0 -> ok;
                                _ ->
                                    db:exec("insert into channel_exchange_log(channel_id, role_id, exchange, type, time) value(?, ?, ?, ?, ?)", [Channel, RoleID, Value, 3, date:unixtime()])
