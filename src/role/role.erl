...
handle_call({reconnect, Pid, Ip, OpenID, RedId, PayId, Icon, NickName}, _From, Role = #role{socket_pid = OldPid, role_id = RoleID, open_id = OpenID1}) ->
    case OpenID1 =:= "" of
        true -> 
            case get(stop) of
                undefined -> ok;
            ...
            link(Pid),
            Pid ! {save_role_pid, self(), RoleID},
            NewRole = Role#role{socket_pid = Pid, ip = Ip, open_id = OpenID, red_openid = RedId, pay_openid = PayId, icon = Icon, name = NickName},
            role_data:update_online_role(NewRole),
            boradcast_mgr:reconnect(NewRole),
            {reply, role_conver:to_login_role(NewRole), NewRole};
        _ ->
...
handle_call({reconnect, Pid, Ip, OpenID, RedId, PayId, Icon, NickName, Phone, PhoneScreat}, _From, Role = #role{socket_pid = OldPid, role_id = RoleID}) ->
    case get(stop) of
        undefined -> ok;
        Ref ->
            erlang:cancel_timer(Ref)
    ...
    link(Pid),
    Pid ! {save_role_pid, self(), RoleID},
    NewRole = Role#role{socket_pid = Pid, ip = Ip, open_id = OpenID, red_openid = RedId, pay_openid = PayId, icon = Icon, name = NickName, phone = Phone, phone_screat = PhoneScreat},
    role_data:update_online_role(NewRole),
    boradcast_mgr:reconnect(NewRole),
    {reply, role_conver:to_login_role(NewRole), NewRole};
...
handle_info(daily_gift_flush, Role) ->
    NewRole = do_gift_flush(Role),
    erlang:send_after(date:next_diff(8, 0, 0) * 1000, self(), daily_gift_flush),
    {noreply, NewRole};
...
%% 0点更新
do_zero_flush(Role = #role{great_match = #great_match{week_score = Score}, week_kill = Kill}) -> 
    Now = date:unixtime(),
    Role1 = clean_bonus(Role),
    Role2 = tomorrow_to_today(Role1),
    Day = date:day_of_the_week(Now),
    NewGreat = case Day of
        1 -> #great_match{};
        _ -> #great_match{week_score = Score}
    end,
    NewKill = case Day of
        3 -> 0;
        5 -> 0;
    ...
    Role2#role{daily_value = [], login_time = Now, daily_kill = 0, week_kill = NewKill, great_match = NewGreat}.
...
do_gift_flush(Role) ->
    Now = date:unixtime(),
    charge:push_1907(1),
    Role#role{daily_gift_type = 1, daily_gift_flush = Now}.
