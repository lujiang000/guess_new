...
handle(1127, _, Role) ->
    case role_lib:do_vip_login_reward(Role) of
        {ok, Type, Num, NewRole} ->
            account_mgr:output(?vip_login_coin, [{Type, Num}]),
...
handle(1133, _, Role) ->
    ...
    case Num =:= 0 of
        true ->
            ...
            {ok, #m_1133_toc{coin = Coin}, NewRole1};
        _ ->
            {false, ?error_share}
    end;
...
%% 使用宝箱
handle(1153, #m_1153_tos{type = Type, num = Num}, Role = #role{}) ->
    case do_use_box(Role, Type, Num) of
        {ok, NewRole, List} ->
            {ok, #m_1153_toc{list = List}, NewRole};
        {false, Reason} ->
            {false, Reason}
    end;
...
%% 使用宝箱
do_use_box(Role = #role{name = Name}, Type, Num) ->
    case role_lib:do_cost(Role, [{Type, Num}]) of
        {ok, NewRole} ->
            List = get_box(Type),
            NewList = do_use_box1(List, Num, []),
            Msg = get_box_msg(NewList, []),
            Msg1 = util:fbin("恭喜玩家【~ts】, 打开【~tsX~w】, 获得: ~ts!", [Name, get_box_name(Type), Num, Msg]),
            boradcast_mgr:boradcast(Msg1),
            {ok, NewRole1} = role_lib:do_add(NewRole, NewList),
            {ok, NewRole1, [#p_assets{type = Item, num = Value}||{Item, Value} <-NewList]};
        {false, Reason} ->
            {false, Reason}
    end.

%% 获取广播消息
get_box_msg([{Type, Num} | L], List) -> 
    Msg1 = lists:concat([get_item_name(Type), "X", Num, " "]),
    get_box_msg(L, Msg1 ++ List);
get_box_msg([], List) -> List.



%% 获取宝箱名字
get_box_name(box1) -> "木宝箱";
get_box_name(box2) -> "银宝箱";
get_box_name(box3) -> "金宝箱".

%% 物品名字
get_item_name(horn) -> "号角";
get_item_name(rage) -> "狂暴";
get_item_name(locking) -> "锁定";
get_item_name(ice) -> "冰冻";
get_item_name(gold) -> "钻石";
get_item_name(coin) -> "金币";
get_item_name(gold_pick) -> "金猪";
get_item_name(auto) -> "自动";
get_item_name(lollipop) -> "波板糖".


%% 打开宝箱
do_use_box1(_List, 0, List) -> List;
do_use_box1(ItemList, Num, List) ->
    {Type, N, _} = sys_rand:rand_list(ItemList, 3),
    List1 = case lists:keyfind(Type, 1, List) of
        {Type, N1} ->
            lists:keyreplace(Type, 1, List, {Type, N + N1});
        _ ->
            [{Type, N} | List]
    end,
    do_use_box1(ItemList, Num - 1, List1).


%% 获取box的物品列表
get_box(box1) ->
    [{horn, 1, 500}, {rage, 1, 900}, {locking, 1, 900}, {ice, 1, 1500}, {gold, 5, 400}, {coin, 10000, 200}];
get_box(box2) ->
    [{horn, 1, 250}, {locking, 2, 150}, {gold_pick, 1, 100}, {auto, 1, 200}, {gold, 10, 200}, {coin, 20000, 100}];
get_box(box3) ->
    [{horn, 1, 6000}, {auto, 1, 3300}, {gold_pick, 1, 500}, {gold, 20, 500}, {coin, 50000, 500}, {lollipop, 1, 105}].
