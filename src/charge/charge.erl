...
%% 每日充值礼包
charge_callback(Role = #role{charge = Charge, parent_id = ParentId, role_id = RoleID, vip = Vip, vip_charge = VipCharge, channel = Channel, daily_gift_type = GiftType}, {Id, Type, Num}) when Type >= ?charge_daily_gift1 andalso Type =< ?charge_daily_gift18 ->
    case db:get_one("select status from charge_log where id = ?", [Id]) of
        {ok, ?charge_status_start} ->
            GiftList = daily_charge_data:get_gift_by_type(GiftType),
            case lists:member(Type, GiftList) of
                true ->
                    NewVip = vip:get_lev(VipCharge + Num),
                    case NewVip =:= Vip of
                        true -> ok;
                    ...
                    #p_daily_gift{list = List} = daily_charge_data:get_gift_by_id(Type),
                    {ok, NewRole} = role_lib:do_add(Role, [{Item, Value} ||#p_assets{type = Item, num = Value} <-List]),
                    Now = date:unixtime(),
                    db:exec("update charge_log set status = ? , call_time = ? where id = ?", [?charge_status_finish, Now, Id]),
                    case Channel of
                        0 -> ok;
                    ...
                    sys_conn:pack_send(1123, #m_1123_toc{type = Type, list = List}),
                    friend_mgr:charge(ParentId, Num),
                    account_mgr:charge(Type, Num, 0),
                    push_1907(GiftType + 1),
                    NewRole1 =  NewRole#role{daily_gift_type = GiftType + 1, charge = Charge + Num, vip_charge = VipCharge + Num, vip = NewVip},
                    {ok, role_lib:do_set_sheep(NewRole1)};
                _ ->
                    charge_callback(Role, {Id, ?charge_coin, Num})
            end;
...
%% 推送下一个每日礼包
push_1907(Type) ->
    List = daily_charge_data:get_gift_by_type(Type),
    List1 = [daily_charge_data:get_gift_by_id(Id) || Id<-List],
    sys_conn:pack_send(1907, #m_1907_toc{type = Type, list = List1}).
