...
%% 获取可以购买的每日礼包信息
handle(1906, _, _Role = #role{daily_gift_type = Type}) ->
    case sys_env:get_env(daily_active) of
        1 ->
            List = daily_charge_data:get_gift_by_type(Type),
            NewList = [daily_charge_data:get_gift_by_id(Id) || Id<-List],
            {reply, #m_1906_toc{type = Type, list = NewList}};
        _ -> 
            {reply, #m_1906_toc{type = 0, list = []}}
    end;
