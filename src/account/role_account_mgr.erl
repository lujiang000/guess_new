...
login(RoleId, Type) ->
    ?MODULE ! {login_type, Type, 0, RoleId}.
...
%% 登陆类型
handle_info({login_type,  wx, Channel, Id}, State) ->
    case catch ets:lookup(role_account_mgr, Channel) of
        [Account = #role_account_mgr{wx_login = Login, wx_login_list = List}] ->
            case lists:member(Id, List) of
                true -> 
                    ok;
                _ -> 
                    ets:insert(role_account_mgr, Account#role_account_mgr{wx_login = Login + 1, wx_login_list = [Id | List]})
            end;
        [] ->
            ets:insert(role_account_mgr, #role_account_mgr{channel = Channel, wx_login = 1, wx_login_list = [Id], time = date:unixtime(zero)});
        _R ->
            ?ERR("注册登陆查询错误：~w", [_R]),
            ok
    end,
    {noreply, State};
handle_info({login_type,  web, Channel, Id}, State) ->
    case catch ets:lookup(role_account_mgr, Channel) of
        [Account = #role_account_mgr{web_login = Login, web_login_list = List}] ->
            case lists:member(Id, List) of
                true -> 
                    ok;
                _ -> 
                    ets:insert(role_account_mgr, Account#role_account_mgr{web_login = Login + 1, web_login_list = [Id | List]})
            end;
        [] ->
            ets:insert(role_account_mgr, #role_account_mgr{channel = Channel, web_login = 1, web_login_list = [Id], time = date:unixtime(zero)});
        _R ->
            ?ERR("注册登陆查询错误：~w", [_R]),
            ok
    end,
...
%% 入库
save([#role_account_mgr{channel = Channel, registe = Registe, login = Login, next_day = Next, seven_day = Seven, time = Time, fifteen_day = Fifteen, wx_login = WxLogin, web_login = WebLogin} | L]) -> 
    %% 今日数据处理
    case db:get_row("select id from role_account_log where channel =  ? and time = ?", [Channel, Time]) of
        {ok, [Id]} ->
            db:exec("replace into role_account_log(id, channel, registe, login, time, wx_login, web_login) values (?, ?, ?, ?, ?, ?, ?)", [Id, Channel, Registe, Login, Time, WxLogin, WebLogin]);
        _ ->
            db:exec("insert into role_account_log(channel, registe, login, time, wx_login, web_login) values (?, ?, ?, ?, ?, ?)", [Channel, Registe, Login, Time, WxLogin, WebLogin])
    end,
    %% 次日数据处理
...
update_var({role_account_mgr, Channel, Regist, Login, LoginList, NextDay, NextList, SevenDay, SevenList, Time, FifteenDay, FifteenList}) ->
    update_var({role_account_mgr, Channel, Regist, Login, LoginList, NextDay, NextList, SevenDay, SevenList, Time, FifteenDay, FifteenList, 0, [], 0, []});
