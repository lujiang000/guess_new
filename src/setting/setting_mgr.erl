...
web_get(Type = ?setting_redbag_exchange) ->
    case ets:lookup(setting, Type) of
        [#setting{value = Value}] -> [{result, Value}];
        _ -> [{result, 0}]
    end;
...
web_get(Type = ?setting_shop_redbag) ->
    case ets:lookup(setting, Type) of
        [#setting{value = {Start, End, _, Num}}] -> [{start_time, Start}, {end_time, End}, {num, Num}];
        _ -> []
    end;

web_get(Type = ?setting_redbag_exchange_num) ->
    case ets:lookup(setting, Type) of
        [#setting{value = {Now, Value}}] -> [{now, Now}, {all, Value}];
        _ -> [{now, 0}, {all, 0}]
    end;
...
web_set(Type = ?setting_redbag_exchange_num, Value) ->
    ets:insert(setting, #setting{type = Type, value = {Value, Value}});
web_set(Type, Value) ->
    ets:insert(setting, #setting{type = Type, value = Value}),
    true.

web_set(Type, Start, End, Num) when is_integer(Start) andalso is_integer(End) andalso is_integer(Num)->
    ets:insert(setting, #setting{type = Type, value = {Start, End, Num, Num}}),
    case Type of
        ?setting_shop ->
            shop:reload();
        ?setting_shop_redbag ->
