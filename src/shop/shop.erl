...
terminate(_Reason, _State = #state{start_time = Start, end_time = End, num = Num, all_num = AllNum}) ->
    ?INFO("正在关闭~w......", [?MODULE]),
    setting_mgr:set(?setting_shop, {Start, End, Num, AllNum}),
    ?INFO("~w关闭完成", [?MODULE]),
    ok.
...
do_reload() ->
    case setting_mgr:get(?setting_shop) of
        {ok, {Start, End, Now, Num}} ->
            {Start, End, ?shop_items, Now, Num};
        _ ->
