...
to_atom_assets(22) -> box1;
to_atom_assets(23) -> box2;
to_atom_assets(24) -> box3.
...
%% 删除邮件系统的邮件
delete_mgr_mail(RoleId) ->
    case dets:lookup(mail, RoleId) of
        List = [#mail{} | _] ->
            [dets:delete_object(mail, Mail)||Mail <-List];
        _ ->
            ok
...
%% 删除玩家身上所有的邮件
delete_role_mail(RoleId) ->
    case role_data:get_role_from_dets(RoleId) of
        {ok, Role = #role{}} ->
            role_data:save(Role#role{mail_list = []});
        _ ->
            ok
    end.

%% 删除玩家身上一封的邮件
delete_role_mail(RoleId, Id) ->
    case role_data:get_role_from_dets(RoleId) of
        {ok, Role = #role{mail_list = List}} ->
            NewList = lists:keydelete(Id, #mail.id, List),
            role_data:save(Role#role{mail_list = NewList});
        _ ->
            ok
    end.
