...
weixin_login({Code, ParentID, RedCode, PayCode, Channel, RoleId, Flag}, State) ->
    case get_weixin_OpendId(Code) of
        {OpenID, NickName, Icon, Sex} ->
             case get_weixin_RedId(RedCode) of
                 {ok, RedId} ->
                     case get_weixin_PayId(PayCode) of
                         {ok, PayId} ->
                             do_login({OpenID, NickName, Icon, Sex, ParentID, RedId, PayId, Channel, RoleId, Flag}, State);
                         _ ->
                             {false, ?error_busy}
                    end;
                _ ->
                            ...
                            case lists:keyfind(<<"headimgurl">>, 1, Json) of
                                {_, Icon} ->
                                    case lists:keyfind(<<"sex">>, 1, Json) of
                                        {_, Sex} ->
                                            {OpenID, NickName, to_high_definition(Icon), Sex};
                                        _ ->
                                            ""
                                    end;
                                _ ->
...
account_login(Account, State) ->
    do_login({Account, lists:concat(["游客", Account]),  sys_rand:rand_list(?default_icon_list), 0, 0, "", "", 0, 0, 0}, State).
...
get_role_id([{_Type, ""} | L]) ->
    get_role_id(L);
get_role_id([{Type, OpenId} | L]) ->
    Sql = case Type of
        openid -> 
            "select role_id from role where openid = ?";
        redopenid -> 
            "select role_id from role where red_openid = ?";
        payopenid -> 
            "select role_id from role where pay_openid = ?"
    end,
    case db:get_one(Sql, [OpenId]) of
...
get_role_id([]) -> {ok, undefined}.


%% 账号登陆，微信登陆都走这里 微信正常登陆
do_login({OpenId, NickName, Icon, Sex, ParentID, RedId, PayId, Channel, 0, _Flag}, State = #conn{ip = IP}) ->
    case get_role_id([{openid, OpenId}, {redopenid, RedId}, {payopenid, PayId}]) of
        {ok, RoleId} when is_integer(RoleId) ->   %% 老玩家
            case role_black:is_black(RoleId) of
                ok ->
                    case role_data:get_online_role(RoleId) of
                        ...
                        _ ->  
                            ...
                            case role_data:get_role_from_dets(RoleId) of
                                {ok, Term} ->
                                    case role_var:update_var(Term) of
                                        {ok, Role = #role{}} ->
                                            {ok, Pid} = role:start(Role#role{socket_pid = self(), ip = IP, screat = Screat, open_id = OpenId, red_openid = RedId, pay_openid = PayId}),
                                            {ok, role_conver:to_login_role(Role), Screat, State#conn{pid_object = Pid, role_id = RoleId}};
                                        _Err ->
                                            ?ERR("数据库读取玩家数据错误~w", [_Err]),
                                            {false, ?error_role_data}
...
%% 绑定微信号
do_login({OpenId, NickName, Icon, Sex, _ParentID, RedId, PayId, _Channel, RoleId, Flag}, State = #conn{ip = IP})  when is_integer(RoleId)->
    case role_black:is_black(RoleId) of
        ok ->
            case db:get_row("select role_id, phone from role where openid = ?", [OpenId]) of
                {ok, undefined} ->   %% openid没有绑定过
                    ...
                    case role_data:get_online_role(RoleId) of
                        {ok, #online_role{pid = Pid, screat = Screat}} ->    %% 在线断线重连
                            case catch  gen_server:call(Pid, {reconnect, self(), IP, OpenId, RedId, PayId, Icon, NickName}) of
                                LoginRole = #p_role_info{} ->
                                    {ok, LoginRole, Screat, State#conn{pid_object = Pid, role_id = RoleId}};
                                {false, Reason} -> 
                                    {false, Reason};
                                ...
                                {ok, Term} ->
                                    case role_var:update_var(Term) of
                                        {ok, Role = #role{open_id = ""}} ->
                                            {ok, Pid} = role:start(Role#role{socket_pid = self(), ip = IP, screat = Screat, sex = Sex, icon = Icon, name = NickName, open_id = OpenId, red_openid = RedId, pay_openid = PayId}),
                                            {ok, role_conver:to_login_role(Role), Screat, State#conn{pid_object = Pid, role_id = RoleId}};
                                        {ok, _} ->
                            ...
                            case db:get_row("select phone, phone_screat from role where role_id = ?", [RoleId]) of
                                ...
                                {ok, [Phone1, PhoneScreat1]} ->
                                    ...
                                    case role_data:get_online_role(RoleId1) of
                                        {ok, #online_role{pid = Pid, screat = Screat}} ->    %% 在线断线重连
                                            case catch  gen_server:call(Pid, {reconnect, self(), IP, OpenId, RedId, PayId, Icon, NickName, Phone, PhoneScreat}) of
                                                LoginRole = #p_role_info{} ->
                                        ...
                                        _ ->  
                                            ...
                                            case role_data:get_role_from_dets(RoleId1) of
                                                {ok, Term} ->
                                                    case role_var:update_var(Term) of
                                                        {ok, Role} ->
                                                            {ok, Pid} = role:start(Role#role{socket_pid = self(), ip = IP, screat = Screat, phone = Phone, phone_screat = PhoneScreat, sex = Sex, icon = Icon, name = NickName, open_id = OpenId, red_openid = RedId, pay_openid = PayId}),
                                                            {ok, role_conver:to_login_role(Role), Screat, State#conn{pid_object = Pid, role_id = RoleId1}};
                                                        _Err ->
                                                            ?ERR("数据库读取玩家数据错误~w", [_Err]),
                                                            {false, ?error_role_data}
                                                    end;
