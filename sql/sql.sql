...
-- alter table role add `channel` int(11)  DEFAULT '0' COMMENT '合作商id' after `tel_fare`;
-- alter table role add `pay_openid` varchar(50) COMMENT '支付openid' after `channel`;
-- alter table role add index `channel` (`channel`) ;
-- alter table role add `icon` varchar(300) COMMENT '头像' after `name`;
-- alter table role add index `parent_id` (`parent_id`) ;
-- alter table role add `max_coin` int(11)  DEFAULT '1' COMMENT '最大挡位' after `pay_openid`;
-- alter table role add `candy` int(11)  DEFAULT '0' COMMENT '糖果' after `max_coin`;
-- alter table role add `lolly` int(11)  DEFAULT '0' COMMENT '中棒棒糖' after `candy`;
-- alter table role modify column icon varchar(300);
-- alter table role add `red_openid` varchar(50) COMMENT '红包openid' after `pay_openid`;


-- alter table role add `phone_screat` varchar(50) COMMENT '电话密码' after `phone`;
...
-- alter table mail_log add `reward_time` int(11) COMMENT '领取时间' after `time`;
-- alter table mail_log add `status` int(11) COMMENT '状态' after `reward_time`;
-- alter table mail_log add `delete_time` int(11) COMMENT '删除时间' after `status`;
...
-- alter table charge_log add `charge_type` int(11) DEFAULT '0' COMMENT '充值支付类型，0微信官方，1威富通，2摇钱树支付宝' after `charge_rmb`;
-- alter table charge_log add index multi(time, role_id, status)
...
-- drop index role_id on coin_cost_log;
-- alter table coin_cost_log add index `role` (`role_id`, `time`);
...
-- alter table red_packet_log add `status` int(11) DEFAULT '0' COMMENT '0提现失败，1提现成功' after `cny`;
...
-- alter table daily_task modify column list text;
...
-- alter table red_bag_log add index time_roleid(time, role_id)
...
CREATE TABLE `role_account_log` (
    ...
    `wx_login` int(11) DEFAULT '0'  COMMENT '微信登陆人数',
    `web_login` int(11) DEFAULT '0'  COMMENT '网页登陆人数',
    `time` int(11) DEFAULT '0'  COMMENT '时间',
    PRIMARY KEY (`id`),
    KEY `time` (`time`),
...
-- alter table role_account_log add `channel` int(11) DEFAULT '0' COMMENT '合作商id' after `id`;
-- alter table role_account_log add index `channel` (`channel`, `time`);
-- alter table role_account_log add `fifteen_day` int(11) DEFAULT '0' COMMENT '15日留存' after `seven_day`;
-- alter table role_account_log add `wx_login` int(11) DEFAULT '0' COMMENT '微信登陆人数' after `fifteen_day`;
-- alter table role_account_log add `web_login` int(11) DEFAULT '0' COMMENT '网页登陆人数' after `wx_login`;
...
-- alter table account_log add `gift_code_coin` int(11) DEFAULT '0' COMMENT '礼品码产出金币' after `vip_login_tel`;
-- alter table account_log add `gift_code_redbag` int(11) DEFAULT '0' COMMENT '礼品码产出红包（毛）' after `gift_code_coin`;
